LycanrocEvosAttacksPointers::
	dw LycanrocEvosAttacks
	dw Lycanroc_MidnightEvosAttacks
	dw Lycanroc_DuskEvosAttacks
	dw Lycanroc_DawnEvosAttacks

MeowthEvosAttacksPointers::
	dw MeowthEvosAttacks
	dw Meowth_AlolaEvosAttacks

PersianEvosAttacksPointers::
	dw PersianEvosAttacks
	dw Persian_AlolaEvosAttacks

RotomEvosAttacksPointers::
	dw RotomEvosAttacks
	dw Rotom_FanEvosAttacks
	dw Rotom_FrostEvosAttacks
	dw Rotom_HeatEvosAttacks
	dw Rotom_MowEvosAttacks
	dw Rotom_WashEvosAttacks

MeowsticEvosAttacksPointers::
	dw Meowstic_MEvosAttacks
	dw Meowstic_FEvosAttacks

GrimerEvosAttacksPointers::
	dw GrimerEvosAttacks
	dw Grimer_AlolaEvosAttacks

MukEvosAttacksPointers::
	dw MukEvosAttacks
	dw Muk_AlolaEvosAttacks

SandshrewEvosAttacksPointers::
	dw SandshrewEvosAttacks
	dw Sandshrew_AlolaEvosAttacks

SandslashEvosAttacksPointers::
	dw SandslashEvosAttacks
	dw Sandslash_AlolaEvosAttacks

GirafarigEvosAttacksPointers::
	dw GirafarigEvosAttacks
	dw Girafarig_BetaEvosAttacks

VulpixEvosAttacksPointers::
	dw VulpixEvosAttacks
	dw Vulpix_AlolaEvosAttacks

NinetalesEvosAttacksPointers::
	dw NinetalesEvosAttacks
	dw Ninetales_AlolaEvosAttacks

ShayminEvosAttacksPointers::
	dw ShayminEvosAttacks
	dw Shaymin_SkyEvosAttacks
