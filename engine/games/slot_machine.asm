...
; This is achieved by throwing Foongus until the desired result
; is produced. The amount of Foongus thrown can be anywhere from
;  1 to 14 for SEVEN bias, and 4-8 for no bias.
	...
	ld hl, REEL_ACTION
	add hl, bc
	inc [hl] ; REEL_ACTION_WAIT_FOONGUS
	ld hl, REEL_SPIN_RATE
	add hl, bc
	ld [hl], 0
	call Slots_GetNumberOfFoonguss
	push bc
	push af
	depixel 12, 13
	ld a, SPRITE_ANIM_INDEX_SLOTS_FOONGUS
...
.one
	ld hl, REEL_ACTION
	add hl, bc
	inc [hl] ; REEL_ACTION_END_FOONGUS
	ld hl, REEL_SPIN_RATE
	add hl, bc
	ld [hl], 8
	ret

ReelAction_EndFoongus:
	xor a
	ld [wSlotsDelay], a
	ld hl, REEL_ACTION
	add hl, bc
	dec [hl] ; REEL_ACTION_WAIT_FOONGUS
	ld hl, REEL_SPIN_RATE
	add hl, bc
	ld [hl], 0
	ret
