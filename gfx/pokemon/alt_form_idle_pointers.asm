LycanrocAnimationIdlePointers:
	dw LycanrocAnimationIdle
	dw Lycanroc_DuskAnimationIdle
	dw Lycanroc_MidnightAnimationIdle
	dw Lycanroc_DawnAnimationIdle

MeowthAnimationIdlePointers:
	dw MeowthAnimationIdle
	dw Meowth_AlolaAnimationIdle

PersianAnimationIdlePointers:
	dw PersianAnimationIdle
	dw Persian_AlolaAnimationIdle

RotomAnimationIdlePointers:
	dw RotomAnimationIdle
	dw Rotom_FanAnimationIdle
	dw Rotom_FrostAnimationIdle
	dw Rotom_HeatAnimationIdle
	dw Rotom_MowAnimationIdle
	dw Rotom_WashAnimationIdle

MeowsticAnimationIdlePointers:
	dw Meowstic_MAnimationIdle
	dw Meowstic_FAnimationIdle

GrimerAnimationIdlePointers:
	dw GrimerAnimationIdle
	dw Grimer_AlolaAnimationIdle

MukAnimationIdlePointers:
	dw MukAnimationIdle
	dw Muk_AlolaAnimationIdle

NidoranAnimationIdlePointers:
	dw Nidoran_MAnimationIdle
	dw Nidoran_FAnimationIdle

SandshrewAnimationIdlePointers:
	dw SandshrewAnimationIdle
	dw Sandshrew_AlolaAnimationIdle

SandslashAnimationIdlePointers:
	dw SandslashAnimationIdle
	dw Sandslash_AlolaAnimationIdle

SawsbuckAnimationIdlePointers:
	dw SawsbuckAnimationIdle
	dw Sawsbuck_SummerAnimationIdle
	dw Sawsbuck_FallAnimationIdle
	dw Sawsbuck_WinterAnimationIdle

GirafarigAnimationIdlePointers:
	dw GirafarigAnimationIdle
	dw Girafarig_BetaAnimationIdle

VulpixAnimationIdlePointers:
	dw VulpixAnimationIdle
	dw Vulpix_AlolaAnimationIdle

NinetalesAnimationIdlePointers:
	dw NinetalesAnimationIdle
	dw Ninetales_AlolaAnimationIdle

UnownAnimationIdlePointers:
	dw UnownAAnimationIdle
	dw UnownBAnimationIdle
	dw UnownCAnimationIdle
	dw UnownDAnimationIdle
	dw UnownEAnimationIdle
	dw UnownFAnimationIdle
	dw UnownGAnimationIdle
	dw UnownHAnimationIdle
	dw UnownIAnimationIdle
	dw UnownJAnimationIdle
	dw UnownKAnimationIdle
	dw UnownLAnimationIdle
	dw UnownMAnimationIdle
	dw UnownNAnimationIdle
	dw UnownOAnimationIdle
	dw UnownPAnimationIdle
	dw UnownQAnimationIdle
	dw UnownRAnimationIdle
	dw UnownSAnimationIdle
	dw UnownTAnimationIdle
	dw UnownUAnimationIdle
	dw UnownVAnimationIdle
	dw UnownWAnimationIdle
	dw UnownXAnimationIdle
	dw UnownYAnimationIdle
	dw UnownZAnimationIdle

AegislashAnimationIdlePointers
	dw AegislashAnimationIdle
	dw Aegislash_BladeAnimationIdle

SuicuneAnimationIdlePointers:
	dw SuicuneAnimationIdle
	dw Suicune_BetaAnimationIdle

RaikouAnimationIdlePointers:
	dw RaikouAnimationIdle
	dw Raikou_BetaAnimationIdle

EnteiAnimationIdlePointers:
	dw EnteiAnimationIdle
	dw Entei_BetaAnimationIdle

ShayminAnimationIdlePointers:
	dw ShayminAnimationIdle
	dw Shaymin_SkyAnimationIdle

ZygardeAnimationIdlePointers:
	dw Zygarde_Ten_PercentAnimationIdle
	dw ZygardeAnimationIdle
	dw Zygarde_Complete
